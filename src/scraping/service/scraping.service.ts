        ...
        new Promise((resolve, reject) => {
            this.page.on('request', request => {
                ...
                if (request.resourceType() === 'xhr' && match) {
                    this.urlFollow = request.url(); // Stocker l'URL de la requête interceptée
                    console.log('Requête XHR interceptée:', this.urlFollow);
                    resolve(this.urlFollow); // Résoudre la promesse avec l'URL interceptée
                }
            });
        }).then((url: string) => {
            ...
            const responseData = await this.page.evaluate((url: string) => {
                // Définir la fonction pour effectuer une requête Axios
                window.axiosRequest = async () => {
                    try {
