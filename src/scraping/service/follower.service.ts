...
export class FollowerService implements IFollowerService {
  ...
  async getAllFollowers(
    ...
    // Code adapté de getAllFollow pour les followers
  }
  ...
  async getFollowersOfUser(
    ...
    // Code adapté de getFollowOfUser pour les followers
  }

  private async callApiAndSaveFollowers(
    url: string,
    userDto: UserDto,
  ): Promise<string> {
    // Code adapté de callApiAndSaveBdd pour les followers
  }

  private async addFollowers(pseudo: string, users: UserDto[]): Promise<void> {
